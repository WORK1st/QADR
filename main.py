...
def run_train(args, d_train_data, d_valid_data, d_word2id, d_entity2id, d_relation2id, keqa_checkpoint_path, d_entity_neighours, d_entity2bucketid, d_action_space_buckets, checkpoint_path, intermediate_result_path):
    ...
    for _ in range(args.num_workers): # TODO:将TrainWorker的参数增加一个d_word2id,d_entity2id，以便将question的tensor转化为完整的问题句子。
        p = TrainWorker(args, len(procs), done, shared_model, optim, train_env, train_gstep, valid_gstep, d_entity_neighours, d_entity2bucketid, d_action_space_buckets, num_episodes, len(d_word2id), len(d_entity2id), len(d_relation2id), keqa_checkpoint_path, d_word2id, d_relation2id, d_entity2id)
        p.start()
        procs.append(p)
...
if __name__ == "__main__":
    ...
    # --train --dataset=PQ-3H --KGE_model=ComplEx --strategy=sample
    if args.train:
        if args.dataset.startswith("MetaQA"):
        ...
        reqa_checkpoint_path = os.path.join(output_path, "best_backup.ckpt")
